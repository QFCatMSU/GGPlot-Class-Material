...
We could plot the seven columns individually, but we want a solution that can be extended to any situation (i.e., any number of columns to plot). We are going to look at two distinct ways to handle this situation.
...
In the reshaped data frame, we put all 217 temperature values from the seven columns in one column. This is called [melting]{.hl} a data frame.  So, the reshaped data frame will have 217 rows.
...
![After generalizing the x mapping, we still have the same plot as before](images/GGP_1-06b_PlotMultColumns.png){#fig-generalizedXMap .fs alt="After generalizing the x mapping, we still have the same plot as before"}
