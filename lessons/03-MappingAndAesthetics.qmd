...
1\) For the linear model in the last plot (the ***geom_smooth()*** component), add a mapping of ***color=season***.
...
2\) Create a scatterplot that maps the four columns , ***windSpeed***, ***precip2***, ***windDir*** and ***tempDept*** ([note: **tempDept** is how far the temperature was from average]{.note})

-   you choose the mappings except use ***shape*** for one (shape will only work for one)
...
3\) In comments in your script answer: What patterns can you find in the plot from #2?

 

4\) Create a scatterplot that shows a relationship between temperature departure (***tempDept***) and some other column. Add a linear model of the relationship to the plot. Label everything appropriately and add one more mapping to the plot.
