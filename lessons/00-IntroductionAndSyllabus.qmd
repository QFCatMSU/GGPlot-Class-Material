...
-   find consistent place to put all GGPlot files

-   data files, along with other linked assets, need to be saved in two places (regular folder, docs folder)

    -   put script and data files directly in docs?
    -   link with direct or relative link?

-   changes between local (main) and github (docs) – best to use URL

-   \<done\> Which module.js gets used?

-   Quarto working directory is not the same as R –

    -   causes problem when you want to link to file in R (e.g., weatherData)

#### Lesson module, editor

-   When a link goes to same page, stop from reloading in website
...
-   [\<done – hacky\> figures on codeblocks]{.note}
...
-   [figures under images will not work if there is not blank lines on both sides of the image command in Source]{.note}
...
-   [copy/paste from D2L causes misalignment of 2nd level bullets -- and the visual editor will treat it as code when switching from Source to Visual]{.note}

-   do better check on links within a page – make sure you are not capturing links to pages within the website

-   \<done\> horizontally resize viewer (grid -\> body-width: 1000px)

-   image change size makes page jump after reload (worse the further you are down the page
...
-   set css to only change object within the Quarto document
...
-   [shortcut to codeblock]{.note}

-   [short to unicode]{.note}

    -   « »

-   [\<put in instructions\> create a newline]{.note}

    -   [one way: \<br\>]{.note}
    -   [another: nbsp (control-space in Visual)]{.note}
    -   [another: \\ in Source]{.note}
...
![](images/belinsky2.JPG){.fs width="800"}
...
![Testing the audio and video in Zoom (click on picture to resize).](images/R-00-Zoom.PNG){#fig-zoomImg .flexSize fig-align="left"}
...
![The long-click menu -- this view is from Firefox .](images/R-00-LongClickMenu.PNG){#fig-shortcutMenu .fs}

### Resizing Pictures

Most pictures in the course can be resized so that the picture is out of your way when you don't need to view it.  Clicking on the picture toggles it between the minimized and maximized states (@fig-NinhBinh).

![Test picture to toggle (Ninh Bình, Việt Nam)](images/NinhBinh%20(Large).jpg){#fig-NinhBinh .fs}
