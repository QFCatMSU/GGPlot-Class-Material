...
![The six plots using the matrix layout](images/GGP-01-10 MatrixLayout.PNG){#fig-byMatrix .fs}
...
Find how different weather conditions in the ***weatherType*** column correlate with ***tempDept***. [Note: **tempDept** is how far the temperature for the day was from the historic average temperature for that day.]{.note}

 

1.   Create a script file in your Class Project called ***app12.r***.

2.   Create a histogram of ***tempDept*** restricted to days that meet a condition in the ***weatherType*** column:

    -   To restrict the data, you can subset with the indices that meet the condition (e.g., ***data=weatherData\[daysWithRain,\]***)

3.  Repeat step 2 for two more conditions in the ***weatherType*** column (so, three histograms in all)

4.   Create a histogram of ***tempDept*** restricted to days where [two conditions occur]{.hl} in the ***weatherType*** column (e.g., rainy AND breezy).

5.   Create a histogram of ***tempDept*** restricted to days where [one of two conditions occur]{.hl} in the ***weatherType*** column (e.g., rainy OR breezy).

6.   For all histograms, place a vertical line at the average ***tempDept*** (of the restricted data) and label the line with the average value

    -   ***geom_vline*** is an option here

7.  Using ***arrangeGrob()***, place the 5 histograms you created in the previous steps on one canvas

8.   Using ***arrangeGrob()***, pick three histograms from steps 2-4 and place them on a canvas
