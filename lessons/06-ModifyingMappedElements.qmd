...
The season are given in alphabetical order, which is the default for GGPlot when no order is given by the caller. Also, the colors are probably not the ones we would choose to represent the seasons.
...
With the four ***levels*** given at the end:
...
For all mapped elements, [there is a scale component](https://ggplot2.tidyverse.org/reference/#scales) that can modify the properties of the mapped element.

 

The [scale components reference page](https://ggplot2.tidyverse.org/reference/#scales) is a bit intimidating at first!
...
This message is a bit aggressive. It says that ggplot uses 30 bins by default and that you can change it. We will do this with ***bins*** and ***binwidth*** later.
...
A gradient from dark blue to light blue is probably not the best choice and you can change this using the component ***scale_color_gradient***.
