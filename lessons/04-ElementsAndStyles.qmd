...
Pretty much everything in a GGPlot canvas can be customized. How you customized a element depends largely on what type of element it is.

 

For styling purposes, there are three types of elements in GGPlot:
...
3.  Non-plot components (everything in the canvas that is not part of the plot, e.g., grid lines)

 

Each of these element categories above have a distinct method of styling.  We will cover #1 and #3 in this lesson and #2 in a later lesson when we customize mappings.
...
[Extension: Saving and plotting GGPlot]
...
We can also use properties like ***color, size***, and ***shape*** to style all the points exactly the same (i.e., irrespective of the data).
...
plot2 = ggplot( data=weatherData ) +
           geom_point( mapping=aes(x=avgTemp, y=relHum),  # mapping
                       «color="darkgreen"», # subcomponent, not a mapping
                       «size=2.5,»          # subcomponent, not a mapping
                       «shape=17» ) +       # subcomponent, not a mapping
           theme_bw() +
           labs(title = "Humidity vs. Temperature",
                subtitle = "Lansing, Michigan: 2016",
                x = "Temperature (F)",
...
-   ***shape***: there are three options for shape
    ...
    -   any character on the keyboard in quotes (so, ***shape = "\~"*** would use the ***tilde*** as a point shape)   
...
![Two line graphs: maxTemp vs day number and minTemp vs day number](images/GGP-1-04_Line_Plots.png){#fig-linePlots .fs}
...
Lines and points both use the properties ***color***, ***fill***, and ***alpha***.  The difference is that point ***shape*** and point ***size***.is replaced with ***linetype*** and ***linewidth***
...
[Note: You can create more customized lines in GGPlot by setting the size and frequency of the dots and dashes, but that involves hexadecimals number and is an advanced topic]{.note}.

### Add a smoothing function

On the graph, we are going to add another component that represents the average temperature (***avgTemp***) and smooth out the values.  Like last lesson, we use ***geom_smooth()***, but we will use a different smoothing method this time (***loess***).
...
![Adding avgTemp as a smoothed plot and styled the line](images/GGP_1-04b_TempPlot.png){#fig-lineColor .fs}
...
***arrow*** is complicated – there are no simple options.
...
***rgb*** stands for red, green, blue and represents the three [primary light colors]{.hl}.  You are probably more familiar with the primary pigment colors, which are red, yellow, blue (more accurately: magenta, yellow, cyan).  The different is that primary light colors combine to make white (they are [additive]{.hl}), whereas the primary pigment colors combine to make black/dark brown (they are [subtractive]{.hl}).
...
[Warning: no font type supports all 280,000+ Unicode characters]{.hl} (many are not even close).  Sticking to more common font makes it more likely that the Unicode character is available.
...
-   The color ***rgb(red=0.7, green=0.5, blue=0)*** is 70% of red light, half green light, and no blue light.  It is yellowish-brown (remember these are light color, not pigment colors)
