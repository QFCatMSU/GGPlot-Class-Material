...
Looking at the ***date*** column in a Viewer tab can see the date is not properly formatted (for instance, it does not have a year):
...
Every value in ***theDate2*** has a year attached to it:
...
Since all three plots, or [***geoms***]{.hl}as GGPlot calls them, use ***dateYr*** for the x-axis, we can move the x-axis mapping to ***ggplot().*** This says that all ***geoms*** will use ***dateYr*** as the ***x*** mapping, meaning we do not have to declare an x-mapping in the individual ***geoms***:
...
In the ***scripts*** folder, I created a new script file named ***ggplot_theming.R*** and inside the script file there is a function named **theme_ugly()** that:

1.  create an object that contains the new theme (I called the object ***newTheme***)

2.  set ***newTheme*** to the ***theme*** changes

    -   In this case, it is ***theme_bw()*** with seven modified subcomponents (just like in @fig-themedPlot)

3.  return ***newTheme*** to the caller of the function
...
There is no function that resets GGPlot to the default geom behavior. Instead you unload the GGPlot package and then reload it.
...
-   The second line loads the GGPlot package (effectively resetting GGPlot back to defaults).
