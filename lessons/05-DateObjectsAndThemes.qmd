...
[note: The **Environment** always shows **Date** values in R's standard date format (YYYY-DD-MM),  which is not the same as the format we used to create the date.]{.note}
...
1.  Create a script file in your Project's ***script*** folder called ***app05.r***. 

2.  Convert this string values into a Date vector: 

    -   stringValues = c("01, January-2019", "02, January-2019", "03, January-2019", "04, January-2019", "05, Januar-2019")

3.  Create a column in ***weatherData*** named ***newDate*** that has the dates from the ***date*** column in Date format (just like ***dateYr*** in the lesson)

4.  Make a GGPlot canvas with 4 geoms
...
6.  Using ***update_geom_defaults(),***on the ggplot from #4

    -   change the point shape to triangles and doubles the size of the points

    -   don't forget to unload the defaults

7.  Create an external theme that:

    1.  right justify the title and subtitle

    2.  make all fonts (titles and labels) brown and arial

    3.  angles the x and y axis test 45 degrees

8.  Add the external theme to the ggplot from #4.

9.  Create a second ggplot that has the same four ***geoms*** but has a legend that contains ***heatDays*** and ***coolDays***
