...
-   Provide a basic overview of the RStudio environment

-   Execute an R script using the Source button
...
On Windows, R and RStudio installation is straightforward and you can use the default settings. For Macs the installation is a bit trickier, the details are below.
...
It is highly recommended that you have version **2025.04** or higher.
...
For now, we are executing the script to test your setup and help you become familiar with the RStudio environment. The code in this script is something you will learn as the class progresses.
...
The script will only execute properly if [it is opened inside your RStudio Project]{.hl}.

### Setting up RStudio tabs for scripts

When we are editing and executing an R script, we generally have the following RStudio tabs open (@fig-openScript):

1.  **File Editor** – text viewer and editor for the opened script files (upper-left pane)

2.  **Console** – displays information about the execution of your script file (lower-left pane)

3.  **Environment** – displays data points, or variables, from the execution of your script file (upper-right pane)

4.  **Files** – the file manager (lower-right pane)

    -   After execution of this script, this will switch to **Plots** (@fig-source) – all plots created by the scrtipt are displayed here
...
[Note: If you have already used R, there is a good chance you highlight lines of code and click **Run** to execute just those lines. This is fine but, in this class, your code must work when executed using **Source**.]{.note}  For more information go to [Extension: Run vs. Source]

![Executing the whole the script (Source)](images/RSProj_Source.png){#fig-source .fs}
...
-   To clean the **Environment,** **Plot**, and **Console** panes, use the brush button (@fig-cleanTabs).

![Cleaning out the panes](images/RSProj_CleanWindows.png){#fig-cleanTabs .fs}
