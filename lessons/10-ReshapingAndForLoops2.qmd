...
2.  Using both reshaping and ***for*** loops, plot the four humidity columns in ***weatherData*** against average temp with wind speed as ***color*** mapping
    ...
    B.  Add titles and labels -- right justify the title

    C.  Correct the legend so that wind speeds are in increasing order and colors goes from lighter to darker blues

    D.  Change the size of the lines and the linetype

3.  Create a text plot of Precipitation (***precip2***) vs Humidity (***relHum***) that is selectively labelled with ***windspeed***

    A.  Map the color of the points to ***windSpeed***

        -   Use a color gradient that goes from green to blue to purple

    B.  Label the points that have precipitation over 0.1in ***and*** humidity greater than 80%
