...
-   [It is best to use the operating system's File Explorer/Finder]{.hl} to move the CSV file to the proper folder. [Trap: Using Excel to move files]
...
@fig-spaceCode shows GGPlot code from the [RStudio Setup lesson](../scripts/RSProj_Setup.R). The GGPlot code has:
...
-   11 subcomponents -- highlighted in red
...
We will be going over the code in @fig-spaceCode in much more detail in later lessons.  For now, we are going to focus on the spacing.  [In this script file](../scripts/lesson02-poor_spacing.R), the GGPlot code was functionally kept the same but the spacing was change to something pretty unreadable. R does not care about spacing, but humans do. [GGplot code for can quickly get unwieldy]{.hl}-- that is why it is important to consider code spacing from the very beginning.  For this class, all components and subcomponents on a GGPlot will get their own line of code.  This makes the code easier to read and is a requirement for all the lesson applications.
...
The same GGPlot code poorly spaced – but it works the same
:::
...
In this class, we will refer to ***labs()*** as a [component]{.hl}and ***title***, ***subtitle***, ***x***, and ***y*** as [subcomponents]{.hl}.

## How to make comments in R

In the R language, comments are created using the number-sign ( ***\#*** ) key.  Any text on the line after the ( ***\#*** ) will be treated by R as a comment and it will not be executed. In @fig-spaceCode, there are comments on lines 1, 2, 3, 7, 12, and 13.  These comments are colored light green but the color of the comments depends on your RStudio color scheme (***Tools -\> Global Options -\> Appearance***).
...
[Note: All the code from this point on in the lesson is also in the script downloaded in [Code for the lesson]]{.note}
...
All the scripts in this class will contain these two lines at the top:
...
The cheat sheet can also be access in ***RStudio -\> Help -\> Cheat Sheets***.
...
There are multiple functions in R and GGPlot where the argument name ***x*** is used generically as the name for the first argument in a function.  This is not intuitive when plotting because ***x*** is refers to data that maps to the x-axis. 
...
2.  What component would you use to change the breaks on the y-axis if the values were in date format?

 

B\) Create a scatterplot of ***stnPressure*** vs ***windSpeed*** using the data from Lansing2016NOAA.csv

1.  Add a title and labels the axes appropriately

2.  Change the plot theme to one of your choice

3.  Change the angle of the ***stnPressure*** axis labels to 45 degrees

4.  Change the ***stnPressure*** breaks to go up by 0.1

5.  Have the **wind-speed** axis only display three values: **3, 12, 21**

6.  Challenge: Use the ***limits*** argument in ***scale_x_continuous*** and ***scale_y_continuous*** to remove the top and right parts of the plot where there are only a couple points.
...
An RStudio Project takes the whole RStudio window -- also called an [RStudio Session]{.hl}.  If you want to open up a second RStudio Project, you need to start a new RStudio Session (i.e., a new RStudio window).  This can be done by clicking ***File -\> Open Project in New Session...*** or ***Session -\> New Session*** and then open the new Project
...
2.  Excel will change the format of certain types of columns.  For instance, if you have a column with values that look like this: ***01-01, 01-02, 01-03*** then Excel will likely switch those values to dates like this: ***Jan-1, Jan-2, Jan-3***
