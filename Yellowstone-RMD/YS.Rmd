...
```{r echo=FALSE, code = readLines("YS.r"), warning=FALSE, message=FALSE, results="hide", fig.show='hide' }
...
<style>
/* It might be easier to use Knitr hyperlinks... */
/* In-page links will still not work in PDF saved from Firefox (works in Chrome, Edge) */
  @media print 
  {
    .pagebreak, .pb
    {
  ...
  .pagebreak, .pb
  {
    /* In chrome and Edge, this needed to be inside a <p>, not a <a> */
    padding-bottom: 50px;
  }
  @page  
  { 
      /* Removing the header/footer is a browser option -- not a CSS option */
      size: letter;   /* not sure this does anything... */ 
      margin-top: 20mm;  
      margin-bottom: 20mm;  
      margin-left: 20mm;  
      margin-right: 20mm;  
  ...
  a:link
  {
    /* !important overrides the default when saving as PDF or printing */         
    color: red !important;
  }
  h2
  {
...
<p class="fig">Figure 2.  Estimates of fully selected instantaneous fishing mortality (F) for suppression gillnetting for Yellowstone Lake lake trout from  **``r yearInit``** through **``r yearEnd``** using a statistical catch-at-age model.  Dashed lines delineate 95% confidence intervals.</p>
<br/><br/><br/>
...
<p class="fig pb">Figure 3.  Abundance estimates for age-2 and older lake trout at the start of the year for **``r yearInit``** to **``r yearEnd``** using a statistical catch-at-age model.  Dashed lines delineate 95% confidence intervals. </p>
...
<p class="fig">Figure 4.  Biomass estimates for age-2 and greater lake trout from **``r yearInit``** through **``r yearEnd``** using a statistical catch-at-age model.  Dashed lines delineate 95% confidence intervals.</p> 
<br/><br/><br/>
...
<p class="fig pb">Figure 5.  Age-2 lake trout abundance in relation to egg abundance, lagged by 2 years.  Labels indicate year class.  The 1998, 1999, and 2000 year classes overlay one another in the bottom left corner of the graph.</p>   
